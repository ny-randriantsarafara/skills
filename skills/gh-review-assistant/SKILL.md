---
name: gh-review-assistant
description: Review GitHub pull requests via GitHub CLI (gh) with a risk-first workflow. Use when the user asks to review PRs, inspect PR comments/reviews, prepare GitHub review feedback, or analyze PR checks with gh. Default to read-only review drafts and only publish comments/reviews after explicit user confirmation.
---
...
2. Collect deterministic PR context.
   ...
   ```bash
   skills/gh-review-assistant/scripts/collect_pr_context.sh <PR_NUMBER_OR_URL>
   ```
